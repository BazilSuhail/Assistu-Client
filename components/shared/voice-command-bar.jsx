...
// Extracted outside so it doesn't recreate every render
const mockResponses = {
  "create task": "Creating a new task for you...",
  "show today": "Showing tasks for today",
  "show overdue": "Displaying overdue tasks",
...
export default function VoiceCommandBar() {
  ...
  const handleVoiceCommand = () => {
    ...
    setTimeout(() => {
      setIsListening(false)
      setResponse(findResponse(input))
    }, 1500)
  }
    ...
    <div className="flex flex-col gap-3">
      <div className="flex gap-2">
        <input
          ...
          placeholder="Try: Create task, Show calendar..."
          className="flex-1 px-4 py-2 rounded-lg bg-input border border-border dark:border-slate-700 dark:bg-slate-700 
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          onClick={handleVoiceCommand}
          className={`p-2 rounded-lg transition-colors ${isListening ? "bg-red-500 text-white" : "bg-primary text-white hover:opacity-90"
            }`}
        >
          <HiMicrophone className="w-5 h-5" />
        </motion.button>
      ...
      {response && (
        <motion.div
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          className="p-3 bg-blue-50 dark:bg-blue-900/20 border border-primary rounded-lg 
