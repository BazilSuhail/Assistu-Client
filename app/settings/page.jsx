...
// Animation variants moved outside the component to avoid re-creation on every render
const containerVariants = {
  hidden: { opacity: 0 },
  visible: { opacity: 1, transition: { staggerChildren: 0.1 } },
}
...
function Toggle({ enabled }) {
  return (
    <div className="relative inline-block w-12 h-6 bg-slate-300 dark:bg-slate-600 rounded-full">
      <input type="checkbox" defaultChecked={enabled} className="absolute opacity-0 w-full h-full cursor-pointer" />
      <motion.div
        layout
        className={`absolute top-1 left-1 w-4 h-4 rounded-full transition-colors ${
          enabled ? "bg-primary translate-x-6" : "bg-white"
        }`}
      />
    </div>
  )
...
export default function SettingsPage() {
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="p-6 md:p-8 max-w-2xl mx-auto">
      <motion.div variants={itemVariants} className="mb-8">
        <h1 className="text-3xl font-bold text-foreground">Settings</h1>
        <p className="text-slate-600 dark:text-slate-400 mt-2">Manage your account and preferences</p>
      </motion.div>
